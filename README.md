...
- Lost connection: the server keeps your seat (colour, scores, strokes) for 90 seconds and
  the tab rejoins by itself. After a page reload the lobby offers "Bli med igjen" instead,
  so a deliberate refresh can start fresh. While you are away the turn skips you, so nobody
  waits on a dropped player. The ⏏ button in the HUD leaves a game.
